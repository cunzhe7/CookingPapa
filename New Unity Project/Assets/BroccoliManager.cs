...
public class BroccoliManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (defeated)
        {
            UI.GetComponentInChildren<Text>().text = "Boss Defeated! Go Cooking!";
            Destroy(smallBoss);
            Destroy(bigBoss);
            Destroy(GameObject.Find("TheEnemy (1)"));
            Destroy(GameObject.Find("TheEnemy (2)"));
            defeated = false;
            goCook = true;
            audio.Play();
            audio.PlayOneShot(bossdied);
            foreach(GameObject a in GameObject.FindGameObjectsWithTag("Respawn"))
            {
                if (a.name == "ballPrefab(Clone)")
                    Destroy(a);
            }
            GameObject resp = GameObject.Find("Ball Spawner");
            Destroy(resp);
        }

        if (Vector3.Distance(wok.transform.position, Player.transform.position) < 2 && goCook)
        {
            inst.text = "Press F to cook";
        ...
        {
            cookMode = false;
            UI.GetComponentInChildren<Text>().text = "You just made the Chicken w. Broccoli successfully!";
